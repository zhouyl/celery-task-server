...
class ConnectionPool(object):
    ...
    def __init__(self, create, max_size=10, max_usage=0, ttl=0, idle=60,
                 block=True, name=None):
        '''初始化参数
            ...
            name:      给连接池一个名称，以便在日志中区分，例如: database/security/memcache
        '''
        ...
        self._size = 0
        self._logger = logger.get('{}.{}'.format(__name__, name or '').rstrip('.'))
    ...
    def item(self):
        ...
        try:
            while (self._max_size and self._pool.empty() and self._size >= self._max_size):
                if not self._block:
                    self._log(logging.WARNING, 'Too many connections')
                    raise TooManyConnections('Too many connections')

                self._log(logging.DEBUG, 'Waiting free connections')
                self._lock.wait()  # 等待闲置连接

            try:
                wrapped = self._pool.get_nowait()  # 从空闲连接池中获取一个
                self._log(logging.DEBUG, 'Using a free connection')
            except queue.Empty:
                wrapped = self._wrapper(self._create())  # 创建新连接
                self._size += 1
                self._log(logging.DEBUG, 'Created new connection')
        finally:
            self._lock.release()
    ...
    def release(self, conn):
        ...
        try:
            ...
            self._log(logging.DEBUG, str(err))
        else:
            self._pool.put_nowait(wrapped)
            self._lock.notifyAll()  # 通知其它线程，有闲置连接可用
            self._log(logging.DEBUG, 'Released a connection')
        finally:
            self._lock.release()
    ...
    def _destroy(self, wrapped):
        ...
        self._size -= 1
        self._log(logging.DEBUG, 'Destroyed a connection')

    def _log(self, level, message):
        '''日志写入，将在 message 后附加上连接池大小及闲置连接数'''
        message += ' [size: %d, free: %d]' % (self._size, self._pool.qsize())
