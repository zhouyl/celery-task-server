...
class Transaction(object):
    ...
    def __enter__(self):
        ...
        self._connection.log(logging.DEBUG, 'Transaction beginning...')
        return self._connection

    def __exit__(self, exc, value, traceback):
        if exc:
            self._connection.log(logging.WARN, 'Transaction rolled back: %s %s', exc, value)
            self._connection.rollback()
            return False

        self._connection.log(logging.DEBUG, 'Transaction committed.')
...
class Connection(pymysql.connections.Connection):
    ...
    def log(self, level, msg, *args, **kwargs):
        '''记录一条日志'''
        return self._logger.log(level, self._log_prefix + str(msg), *args, **kwargs)
...
class ConnectionPooled(object):
    ...
    def __init__(self, pool_options=None, **kwargs):
        ...
        self._pool = pools.ConnectionPool(self._connect,
                                          **{**self._defaults, **pool_options})
